import { BUSINESS_ACTION_LABELS } from "./businessData.js";
    ...
    <div class="business-card">
      ...
      <section class="business-panel business-cooking" data-business-cooking hidden>
        ...
        <div class="business-cooking-actions">
          <button class="primary-button" type="button" data-business-action-success><span>完成动作</span></button>
          <button class="secondary-button" type="button" data-business-action-fail>手忙脚乱</button>
        </div>
      </section>
...
export function createBusinessMode({
  ...
  const refs = {
    ...
    actionSuccess: overlay.querySelector("[data-business-action-success]"),
    actionFail: overlay.querySelector("[data-business-action-fail]"),
  ...
  function renderOrders(snapshot) {
    ...
    snapshot.orders.forEach((order) => {
      ...
      button.innerHTML = `
        ...
        <b>+${order.baseCoins} 金币 · 满意 +${order.satisfactionReward} · 混乱 +${order.chaosRisk}</b>
      `;
  ...
  function renderCooking(snapshot) {
    ...
    refs.cookingMeta.textContent = `动作 ${snapshot.currentActionIndex + 1}/${order.actions.length}：${BUSINESS_ACTION_LABELS[snapshot.currentAction] || snapshot.currentAction}`;
  ...
  function renderUpgrades(snapshot) {
    refs.lastReward.textContent = snapshot.lastReward
      ? `上一单：+${snapshot.lastReward.coins} 金币 · 满意 +${snapshot.lastReward.satisfaction} · 混乱 +${snapshot.lastReward.chaosDelta}${snapshot.lastReward.reasons.length ? ` · 触发 ${snapshot.lastReward.reasons.join(" / ")}` : ""}`
      : "选择一个厨具，决定这局的经营路线。";
  ...
  function render() {
    ...
    mountMascot?.(overlay.querySelector(".business-header"), snapshot.state === "ended" ? "happy" : "idle");
  }

  refs.actionSuccess.addEventListener("click", () => {
    game.completeAction({ success: true });
    render();
  });
  refs.actionFail.addEventListener("click", () => {
    game.completeAction({ success: false });
    render();
  });
