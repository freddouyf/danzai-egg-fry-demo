...
test("磁吸锅铲会在进入绿色区域时自动出锅", () => {
  ...
  game.autoServeStageCharges = 1;
  game.autoServeStageActive = true;
  game.update(2_000);
...
test("自动出锅关卡数最多累积到五关", () => {
  assert.equal(addAutoServeCharges(0, 2), 2);
  assert.equal(addAutoServeCharges(4, 2), MAX_AUTO_SERVE_STAGES);
  assert.equal(addAutoServeCharges(5, 2), MAX_AUTO_SERVE_STAGES);
});
